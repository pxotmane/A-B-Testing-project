...
SELECT DATE_TRUNC('month', occurred_at) AS date_order, SUM(gloss_qty) AS GlossQty, a.name AS NameACCOUNT
FROM accounts a
JOIN orders o ON o.account_id = a.id
GROUP BY date_order, NameACCOUNT
HAVING a.name = 'Walmart'
ORDER BY GlossQty DESC
LIMIT 1
/*Udacity solution*/
...
SELECT account_id, total_amt_usd, CASE WHEN total_amt_usd > 3000 THEN 'Large' ELSE 'Small' END AS level_orders
FROM orders /*6912*/

/* 2-Write a query to display the number of orders in each of three categories, based on the total number of items in each order. The three categories are: 'At Least 2000', 'Between 1000 and 2000' and 'Less than 1000'.*/
SELECT CASE
        WHEN total > 2000 THEN 'At least 2000' /*510*/
        WHEN total > 1000 AND total <= 2000 THEN 'Between 1000 and 2000' /*6332*/
        ELSE 'Less than 1000' /*70*/
...
SELECT a.name,
      CASE WHEN total_amt_usd > 200000 THEN 'greater than 200,000 usd'
      WHEN total_amt_usd > 100000 AND total_amt_usd <= 200000 THEN 'between 200,000 AND 100,000 usd'
      WHEN total_amt_usd <= 100000 THEN  'under 100,000 usd'
      END AS total_spent_category,
      SUM(total_amt_usd) AS total_spent
FROM accounts a
JOIN orders o ON o.account_id = a.id
GROUP BY a.name, total_spent_category
ORDER BY total_spent DESC /*351*/
/*Udacity solution*/
...
/*04-We would now like to perform a similar calculation to the first, but we want to obtain the total amount spent by customers only in 2016 and 2017. Keep the same levels as in the previous question. Order with the top spending customers listed first.*/
SELECT a.name,
      CASE WHEN total_amt_usd > 200000 THEN 'greater than 200,000 usd'
      WHEN total_amt_usd > 100000 AND total_amt_usd <= 200000 THEN 'between 200,000 AND 100,000 usd'
      WHEN total_amt_usd <= 100000 THEN  'under 100,000 usd'
...
SELECT s.name, COUNT(*) AS orders_num, SUM(o.total_amt_usd) AS orders_amt, CASE
WHEN COUNT(*) > 200 OR SUM(o.total_amt_usd) > 75000 THEN 'HIGH GROUP'
WHEN (COUNT(*) <= 200 AND COUNT(*) > 150 ) OR (SUM(o.total_amt_usd) <= 75000 AND SUM(o.total_amt_usd) > 50000) THEN ' MIDDLE GROUP'
WHEN COUNT(*) <= 150 OR SUM(o.total_amt_usd) <= 50000 THEN 'LOW GROUP' END AS group_sales_reps
FROM sales_reps s
JOIN accounts a ON a.sales_rep_id=s.id
JOIN orders o ON o.account_id=a.id
GROUP BY 1
